"""
API 密钥路由单元测试

测试密钥 CRUD 和权限控制。
"""

import sys
import os
import json
import pytest
from unittest.mock import MagicMock, patch
from contextlib import contextmanager
from datetime import datetime, timezone

sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from auth import create_token


def _make_mock_db(mock_conn):
    @contextmanager
    def fake_get_db():
        yield mock_conn
    return fake_get_db


@pytest.fixture
def setup(mock_conn, mock_cursor):
    fake_db = _make_mock_db(mock_conn)
    patches = [
        patch('db.get_db', fake_db),
        patch('routes.auth.get_db', fake_db),
        patch('routes.menus.get_db', fake_db),
        patch('routes.etl_tasks.get_db', fake_db),
        patch('routes.dynamic.get_db', fake_db),
        patch('routes.page_configs.get_db', fake_db),
        patch('routes.users.get_db', fake_db),
        patch('routes.relations.get_db', fake_db),
        patch('routes.api_keys.get_db', fake_db),
        patch('db.pool', MagicMock()),
        patch('utils.operation_log.log_operation'),
    ]
    for p in patches:
        p.start()

    from app import app
    app.config['TESTING'] = True
    admin = create_token({'id': 'u1', 'username': 'admin', 'role': 'admin'})
    dev = create_token({'id': 'u2', 'username': 'dev', 'role': 'developer'})

    yield (
        app.test_client(),
        mock_cursor,
        {'Authorization': f'Bearer {admin}'},
        {'Authorization': f'Bearer {dev}'},
    )

    for p in patches:
        p.stop()


now = datetime(2024, 1, 1, tzinfo=timezone.utc)


class TestListApiKeys:
    def test_admin_can_list(self, setup):
        client, mock_cursor, admin_h, _ = setup
        mock_cursor.fetchall.return_value = [
            ('ak-1', '密钥1', now, None, True),
        ]
        resp = client.get('/apiKeys', headers=admin_h)
        assert resp.status_code == 200
        data = resp.get_json()
        assert len(data) == 1

    def test_developer_forbidden(self, setup):
        client, _, _, dev_h = setup
        resp = client.get('/apiKeys', headers=dev_h)
        assert resp.status_code == 403


class TestCreateApiKey:
    def test_missing_name(self, setup):
        client, _, admin_h, _ = setup
        resp = client.post('/apiKeys',
                           data=json.dumps({'name': ''}),
                           content_type='application/json',
                           headers=admin_h)
        assert resp.status_code == 400

    def test_create_success(self, setup):
        client, mock_cursor, admin_h, _ = setup
        mock_cursor.fetchone.return_value = ('ak-new', '新密钥', now, None, True)
        resp = client.post('/apiKeys',
                           data=json.dumps({'name': '新密钥'}),
                           content_type='application/json',
                           headers=admin_h)
        assert resp.status_code == 201
        data = resp.get_json()
        assert 'key' in data


class TestToggleApiKey:
    def test_toggle_success(self, setup):
        client, mock_cursor, admin_h, _ = setup
        mock_cursor.fetchone.return_value = ('ak-1', '密钥1', now, None, False)
        resp = client.put('/apiKeys/ak-1',
                          data=json.dumps({'isActive': False}),
                          content_type='application/json',
                          headers=admin_h)
        assert resp.status_code == 200

    def test_not_found(self, setup):
        client, mock_cursor, admin_h, _ = setup
        mock_cursor.fetchone.return_value = None
        resp = client.put('/apiKeys/nonexistent',
                          data=json.dumps({'isActive': True}),
                          content_type='application/json',
                          headers=admin_h)
        assert resp.status_code == 404

    def test_missing_param(self, setup):
        client, _, admin_h, _ = setup
        resp = client.put('/apiKeys/ak-1',
                          data=json.dumps({}),
                          content_type='application/json',
                          headers=admin_h)
        assert resp.status_code == 400


class TestDeleteApiKey:
    def test_not_found(self, setup):
        client, mock_cursor, admin_h, _ = setup
        mock_cursor.fetchone.return_value = None
        resp = client.delete('/apiKeys/nonexistent', headers=admin_h)
        assert resp.status_code == 404

    def test_delete_success(self, setup):
        client, mock_cursor, admin_h, _ = setup
        mock_cursor.fetchone.return_value = ('密钥1',)
        resp = client.delete('/apiKeys/ak-1', headers=admin_h)
        assert resp.status_code == 200
