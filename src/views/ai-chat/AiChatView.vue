...
// Sidebar section collapse (会话/批任务/AI定时任务), persisted so a user's
// choice survives reload/re-login — mirrors app.ts's sidebarCollapsed pattern.
const collapsedSections = reactive(getStorage(STORAGE_KEYS.AI_CHAT_SIDEBAR_SECTIONS, {
  sessions: false, batches: false, scanBatches: false,
}))
function toggleSection(key: keyof typeof collapsedSections) {
  collapsedSections[key] = !collapsedSections[key]
  setStorage(STORAGE_KEYS.AI_CHAT_SIDEBAR_SECTIONS, collapsedSections)
}
// 批任务 vs AI定时任务：the batch itself carries scan_task_id (stamped by
// ai_scan_engine.run_task's create_batch call) when it was auto-generated by
// a scheduled scan, as opposed to a user hitting "新建" or the open API.
const regularBatches = computed(() => batches.items.filter(b => !b.scan_task_id))
const scanBatches = computed(() => batches.items.filter(b => !!b.scan_task_id))
...
// via _reloadPersisted. A user continuation hands the child over to the
// ordinary SSE lifecycle for good.
    ...
    <aside class="ai-chat__sidebar">
      <div class="ai-sidebar__sessions-wrap">
        ...
        <ElScrollbar class="ai-chat__sessions">
          ...
          <div v-show="!collapsedSections.batches">
            <BatchGroup
          ...
          <div class="ai-sidebar__section-head" @click="toggleSection('scanBatches')">
            <ElIcon class="caret"><ArrowRight v-if="collapsedSections.scanBatches" /><ArrowDown v-else /></ElIcon>
            <ElIcon class="section-icon"><Clock /></ElIcon>
            AI定时任务
          </div>
          <div v-show="!collapsedSections.scanBatches">
            <BatchGroup
...
.ai-sidebar__section-head {
  ...
  margin: 10px 8px 4px; padding: 2px 4px; border-radius: 4px;
  font-size: 12px; color: var(--el-text-color-secondary); font-weight: 600;
  cursor: pointer; user-select: none;
}
.ai-sidebar__section-head:hover { background: var(--el-fill-color-light); }
.ai-sidebar__section-head .caret { font-size: 12px; }
.ai-sidebar__section-head .section-icon { font-size: 12px; }
.ai-sidebar__section-head .el-button { margin-left: auto; }
