# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Commands

### Development
```bash
# Install dependencies
npm install
pip install flask flask-cors psycopg2-binary PyJWT pytest apscheduler

# Initialize database (runs DDL/schema migrations always; seeds default admin,
# the 巡检管理 demo menu/pages/records, and a demo kefu instance at /kefu/demo
# ONLY on a genuinely fresh database — gated on page_configs/kefu_instances
# being empty, so deleting demo content on an already-initialized DB and
# re-running this for a schema migration will NOT resurrect it)
cd server && python init_db.py

# Start development (both frontend and backend)
npm run dev:all
# Frontend: http://localhost:5173
# Backend: http://localhost:3002

# Run individually
npm run server  # Backend only (port 3002)
npm run dev     # Frontend only (port 5173, proxies /api to :3002)
```

### Testing
```bash
# Frontend (Vitest)
npm run test              # Run all frontend tests
npm run test:watch        # Watch mode

# Backend (Pytest)
npm run test:server       # Run all backend tests

# Both frontend and backend
npm run test:all

# Run single test file
npx vitest run src/stores/__tests__/menu.test.ts
cd server && set PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 && python -m pytest tests/test_backup.py -v
# (PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 is required on Windows — matches `npm run test:server`)
```

### Build
```bash
npm run build  # vue-tsc type check + vite build → dist/
```

## Architecture Overview

This is a **configuration-driven** dynamic data management platform. Unlike traditional CRUD apps, you do **not** create new Vue pages or database tables for new business entities. Instead, you define a `PageConfig` (field schema) and a `Menu` entry, and the system auto-generates the UI and API endpoints.

**Tech stack**: Vue 3 + TypeScript + Element Plus + Pinia (frontend), Python Flask + psycopg2 (backend), PostgreSQL with JSONB (database).

### Key Architectural Patterns

1.  **Single-Table Dynamic Data**: All business data is stored in `dynamic_data` table using PostgreSQL JSONB. The `collection` field (derived from `pageId`) separates entities. This eliminates the need for schema migrations when adding new entities or fields.

2.  **Frontend Dynamic Rendering**: `src/views/dynamic/DynamicPage.vue` is the only page for business data. It reads `pageId` from the route, fetches the `PageConfig` schema, and renders three view modes based on field definitions:
    *   **Table view** (default): `DataTable` + `DynamicForm` dialog
    *   **Kanban view**: `KanbanBoard` with configurable group/card fields
    *   **Excel view**: `ExcelView` using Univer.js spreadsheet

3.  **Dynamic Route Generation**: `src/router/dynamicRoutes.ts` reads the `menus` table at runtime and generates Vue Router routes pointing to `DynamicPage.vue`.

4.  **Field-Driven Logic**: Behavior is determined by `FieldConfig` in `page_configs.fields`. For example:
    *   `controlType: 'relation'` triggers M:N relationship handling via `data_relations` table.
    *   `controlType: 'autoSequence'` triggers sequence generation logic in the store.
    *   `controlType: 'reference'` creates a 1:N link and inherits parent fields into child records.
    *   `controlType: 'quoteSelect'` creates a single-direction multi-select reference.

### All Control Types

Defined in `src/types/field.ts`. The `controlType` value determines which component renders the field:

| Type | Description |
|------|-------------|
| `text`, `textarea`, `richText`, `number` | Input controls |
| `markdown` | Markdown editor (md-editor-v3 `MdEditor` with live preview); stored as Markdown source, rendered read-only via `MdPreview` in the detail dialog + plain-text preview in table cells |
| `select`, `multiSelect`, `radio`, `checkbox` | Selection controls |
| `date`, `datetime` | Date/time pickers |
| `file`, `image` | Upload controls |
| `relation` | M:N bidirectional association via `data_relations` table |
| `reference` | 1:N parent-child with field inheritance |
| `quoteSelect` | Single-direction multi-select quote |
| `autoSequence` | Auto-incrementing ID (e.g., "IC-001") |
| `autoTimestamp` | Auto-filled on create/update |
| `compositeText` | Read-only, auto-computed by joining other field values with a configurable separator (`compositeTextConfig.sourceFields`) |

Fields may also carry a `workflowConfig` (transitions, role gating, conditions, and actions) — used to drive status-style fields through a state machine. See `WorkflowConfig` in `src/types/field.ts`.

### Project Versioning System

Project-level branching for isolating work across multiple data pages within a project:

*   **Branch Creation**: `project_versions` table tracks branches with `projectMenuId`. When creating a branch, records from all collections under that project menu are copied with the new `branch_id`.
*   **Branch Locking**: Branches can be locked (`isLocked`, `lockedBy`, `lockedAt`) to prevent modifications during merge preparation.
*   **Merge Flow**: Merge copies branch records to `main` branch, handling conflicts via `MergeStrategy` (`theirs` = branch wins, `ours` = main wins).
*   **Delete Protection**: Branches referenced by cross-project dependencies cannot be deleted (checked via `check_branch_delete_protection`).
*   **Post-Merge Validation**: After merge, dependent project dependencies are re-validated and notifications sent if state changed.

Key files: `server/utils/project_version.py` (merge logic, branch CRUD), `server/routes/project_versions.py` (API), `src/types/version.ts`.

### Cross-Project Dependencies

Three dependency types for coordinating work across projects:

| Type | Behavior |
|------|----------|
| `track-main` | Auto-follow target project's main branch. No merge blocking. |
| `read-write` | Coordinated branch pairing. Merge blocked if target branch not merged. |
| `read-only` | Pinned to specific version. No blocking, version must exist. |

Validation triggers: on dependency creation/update, after target project merge, periodically via scheduler. Notifications (`dependencyBroken`, `dependencyWarning`, `dependencyResolved`) sent to source project admins on state changes.

Key files: `server/utils/cross_project_dependency.py` (validation logic), `server/utils/dependency_scheduler.py` (hourly validation), `server/utils/notifier.py` (notifications), `server/routes/cross_project_dependencies.py` (API).

### Webhook Rules System

Event-driven webhooks with HMAC-SHA256 signature verification:

*   **Trigger Events**: `create`, `update`, `delete`, `merge` on specified collections, plus `manual` — never auto-fired by CRUD, only invocable by id (see Row Actions below). A `manual` rule ignores its own `triggerCondition`/`triggerTiming`/`rollbackOnFailure` (the management UI hides those fields when `triggerEvent==='manual'`); the calling site owns the condition instead.
*   **Trigger Conditions**: Optional JSON filter for conditional triggering (e.g., `{"status": "completed"}`). Not applicable to `manual`.
*   **Execution**: Retry logic with configurable timeout. Logs stored in `webhook_logs`.
*   **Signature**: `X-Webhook-Signature` header contains HMAC-SHA256 of payload with rule's secret.

Key files: `server/utils/webhook_engine.py` (execution, retry, signing; `fire_webhook_rule` fires a single rule by id for `manual` callers), `server/routes/webhooks.py` (API), `src/types/webhook.ts`.

### 数据页自定义行级操作按钮（Row Actions）

数据页可在行「⋯」菜单里配置自定义按钮，点击后对该行**异步**触发一条 Webhook 规则或一个 AI 扫描任务，执行状态与结果回写到该行字段。

*   **配置**：`page_configs.row_actions`（JSONB 列，与 `view_config` / `delete_binding` 同构），在「页面配置管理 → 行操作」标签页编辑。每个动作是一层**薄绑定**：绑哪个执行器 + 角色可见性 + 单条件显示 (`visibleWhen`) + 二次确认 + 参数表单 (`paramFields`，复用 `FieldConfig`/`DynamicForm`，禁用 `relation`/`reference`/`quoteSelect`/`autoSequence`——这些控件要落库才有语义) + 状态字段与回写映射（`responseMapping`，同构于 `ai_scan_tasks.fieldMapping` 的 `FieldMappingRow`）。
*   **执行器全部复用**：webhook 走 `webhook_engine.fire_webhook_rule`（要求规则 `triggerEvent='manual'`）；AI 走 `ai_scan_engine.claim_one`（`claim_records` 的单行版，**不带 `pendingValue` 谓词**，所以已处理的行能重跑）+ `create_batch` 单子会话（命名 `行操作·<按钮名>·<时间戳>`），回写仍由 `on_child_finished` 完成。**AI 动作一律使用所绑扫描任务自身的 `statusField`/三个状态值**——行动作自己的这五项（`statusField`/`runningValue`/`doneValue`/`failedValue`/`responseMapping`）只对 webhook 生效；配置 UI 在切到 AI 类型时隐藏并清空它们，因为终态回写由扫描任务的引擎完成，叠两套状态字段配置会留下一个永远卡在「执行中」、无人写终态的字段。
*   **编排** `server/utils/row_action_engine.py`（`run_action`）：后端复核角色白名单 + `visibleWhen` → 幂等闸门 → 分派 → 落终态。**两条分支的闸门机制不同**：
    *   Webhook 闸门是真正的**原子 CAS**：`claim_for_webhook` 把判断和写入压进同一条条件 `UPDATE`（`WHERE data->>field IS DISTINCT FROM running OR updated_at < stale_before`），靠 **rowcount** 判断是否抢到，而不是「读后写」——避免并发请求都读到「未在执行」的快照后双双触发外部 webhook。陈旧阈值 = 该规则 `timeout × (retries + 1)`，下限 5 分钟；超过视为进程崩溃留下的孤儿，放行重新触发。
    *   AI 闸门基于路由层在请求开始时已读到的行快照做前置拦截（非原子，`row_action_engine.run_action` 里 `current == running` 的比较用的是调用方传入的 `row_data`），真正的原子认领由 `claim_one` 的 `FOR UPDATE SKIP LOCKED` 完成；**不做陈旧时间放行**（AI 常规要跑数分钟，按时间放行会给同一条记录并发出第二个子会话），孤儿由 `ai_scan_engine.sweep_orphans`（仅在后端进程启动时执行一次）清扫恢复。
*   **端点** `POST /<collection>/<recordId>/row-actions/<actionId>/run`（`dynamic_bp`，`write_required` + `require_page_action(collection,'update')`）。前端 `RowActionRunner.vue` 提交后按 5s 间隔轮询该行状态，上限 5 分钟。
*   **刻意不建执行记录表**：webhook 审计看 `webhook_logs`，AI 审计看子会话对话；行上的状态字段只回答「现在怎么样」。
*   条件求值双份实现（`server/utils/row_action_condition.py` + `src/utils/rowActionCondition.ts`），共读夹具 `server/tests/fixtures/row_action_conditions.json` 防漂移；比较一律走文本化（`_as_text`），并对浮点数做整数归一（`3.0` vs `3`）避免前后端分叉误判。
*   用户文档：`docs/user-guide/data/row-actions.md`。

### AI Agent Chat (M1)

A Claude-style chat drawer that connects to an **OpenCode** agent runtime (not a raw LLM). The browser talks only to Flask; Flask is the gateway and SSE proxy, and OpenCode reaches platform capabilities through a **standalone MCP server** (decoupled from Flask).

*   **Flow**: Flask creates a per-session workspace dir + an opaque session token, asks OpenCode (`opencode serve` HTTP API) to start a session bound to that workspace, and registers the MCP url `http://<mcp>/mcp?token=<token>` with OpenCode. The MCP server validates the token by DB lookup (supports revocation) and derives the user/role for RBAC.
*   **Streaming**: `GET /ai/chat/sessions/:id/events` is a `text/event-stream` proxy over OpenCode's SSE; assistant messages are persisted to `ai_chat_messages` on `message.finished`. The frontend `EventSource` auto-reconnects (1s→2s→5s→10s).
*   **Design intent**: adding a skill to OpenCode grants the frontend capability with zero frontend changes (generic tool-call rendering). M1 renders tool calls as plain JSON; the `ToolCallBubble` + `tool-renderers` registry is deferred to M2.

Key files: `server/routes/ai_chat.py` (`ai_chat_bp`: sessions/messages/SSE/delete), `server/utils/opencode_client.py`, `server/utils/workspace.py` (path-traversal-safe per-session dirs), `server/utils/session_token.py`, `mcp-server/` (standalone FastAPI + MCP Streamable-HTTP service, run from its own `.venv`), `src/components/ai-chat/` (drawer, message list/item, markdown view, input), `src/stores/aiChat.ts`, `src/api/aiChat.ts`. Design + plan: `docs/superpowers/specs/2026-05-26-ai-agent-frontend-design.md`, `docs/superpowers/plans/2026-05-26-ai-agent-frontend-m1.md`. E2E smoke: `e2e/ai-chat-smoke.spec.ts` (`npm run test:e2e`, requires OpenCode + MCP running). 长期记忆（M1）：`server/utils/memory.py` 用 mem0（Chroma + DashScope 兼容端点）按 `user_id` 管理跨会话记忆；`send_message` 转发前注入、SSE `idle` 落库后由 `extract_from_turn` 后台抽取；开关 `ai_settings.mem0_enabled`。全程降级，记忆层异常不影响聊天。M2：MCP 工具 `memory_search/add/delete`（`mcp-server/tools/memory.py`）经 Flask 内部端点 `/ai/memory/internal/*`（`MCP_INTERNAL_TOKEN` 鉴权，`routes/ai_memory_internal.py`）转发，Flask 仍是 Chroma 唯一写入方。M3：`send_message` 在 `send_prompt_async` 抛错时调 `_recover_session_and_resend`（新建 OpenCode session + 注入最近历史摘要 + 更新 `opencode_session_id` + 重发），实现会话韧性恢复；恢复失败则抛原错（不无限重试）。M4：`GET/DELETE /ai/memories`（`routes/ai.py`，删除前校验归属）+ 前端 `MemoryManager.vue`（AI 助手「我的记忆」抽屉）供用户查看/删除自己的记忆。手动补写：`POST /ai/memories`（`add_memory_text(user_id, text, infer)`，写当前用户；默认 `infer=True` 走 mem0 提炼，`verbatim=True`→`infer=False` 原样、仍嵌入）；属管理动作、不受启用开关影响，apiKey 缺失返回 409；前端「我的记忆」抽屉顶部补写区带「原样保存」开关。向量库与数据页上传文件随系统备份：`utils/backup.py` 把 `MEM0_STORE_ROOT`（`vector_store/`）与 `DATA_FILES_ROOT`（`data_files/`）打进备份 ZIP 并整目录还原（`include_vector_store`/`include_data_files`，默认开；通用 `_add_tree_to_zip`/`_restore_tree`。AI 工作区 `AI_WORKSPACE_ROOT` 为临时数据，故意不纳入）。会话治理与审计：个人可 `close`/`reopen`（软状态，保留 token/workspace/OpenCode session，故可重开）、`clear`（`POST /ai/chat/sessions/:id/clear`：原地重置——删旧 OpenCode session + rmtree 工作区并重建脚手架 + 重签 token + 新建 OpenCode session + 删光 `ai_chat_messages` + 置 `active`；会话**保留在列表中可立即续用**，历史与工作区文件不可恢复）或 `delete`（`DELETE /ai/chat/sessions/:id`：置 `status='deleted'` + 吊销 token + 删 OpenCode session + 清工作区；从列表消失、不可 reopen，DB 行/消息保留供审计），管理员（`admin.ai_chat_admin`）可 `GET /ai/chat/admin/sessions` 列出全部会话并 `POST /ai/chat/sessions/:id/archive` 归档；会话生命周期（create/close/reopen/clear/delete/archive）写入 `operation_logs`（`target_type='ai_chat_session'`）做审计溯源。可观测性：`utils/logging_setup.py` 在 `app.py` 启动时（pytest 下跳过文件 handler）统一配置 logging（控制台 + 轮转文件 `server/ai-chat.log`，env `AI_CHAT_LOG_LEVEL`/`AI_CHAT_LOG_FILE`/`AI_CHAT_LOG_MAX_BYTES`/`AI_CHAT_LOG_BACKUP_COUNT`），并把 AI-chat 关键路径打点（`routes/ai_chat.py` send_message 转发/recover、SSE 流 open/close/error；`utils/opencode_client.py` create_session/send_prompt_async 的发起/接受/失败+耗时；`utils/chat_persist.py` 持久化监听线程 start/end/**crash**——此前 `except: pass` 静默吞掉、是"会话卡住无痕"的头号原因，现改 `logger.exception` 留痕），用于定位会话卡住。子代理执行轨迹可见性：OpenCode 委托子代理时产出的 `SubtaskPart`（`type:'subtask'`）持久化在独立的 `ai_chat_subtasks`/`ai_chat_subtask_messages` 表（`id` 就是 OpenCode 自己的子会话 sessionID，不发明并行 id）；共享的 part 映射在 `server/utils/opencode_parts.py`（`map_part`/`format_opencode_error`，`chat_persist.py`/`batch_engine.py` 都调它）；两个只读端点 `GET /ai/chat/sessions/:sid/subtasks/:subtaskId/messages`（`routes/ai_chat.py`，所有权校验）与 `GET /ai/chat/admin/batches/:batchId/sessions/:sid/subtasks/:subtaskId/messages`（`routes/ai_batch_admin.py`，管理员）共用 `utils/subtask_repo.py::get_subtask_messages`；嵌套深度上限 5 层（`MAX_SUBTASK_DEPTH`，`chat_persist.py`/`batch_engine.py`/前端各自定义同名常量）；前端 `src/components/ai-chat/SubtaskBubble.vue` 是递归组件，展开时才惰性拉取内容；`src/stores/aiChat.ts` 的 SSE `message.part.updated` 分支在 `part.type==='subtask'` 时立即 upsert 一条 `status:'running'` 的占位气泡，让委托发生的瞬间就能看到，真实状态由整轮结束后 `_reloadPersisted` 用服务端已按最新状态映射过的持久化内容覆盖。**子会话 id 的发现（task-7 报告 C-1/C-2 已修复，见 `.superpowers/sdd/2026-08-12-subagent-trace-visibility/task-7-report.md`）**：`SubtaskPart.sessionID` 实为 OpenCode `PartBase` 的标准"所属会话"字段（等于父/根会话自己），真正的子会话 id 在配套 `tool` part（`tool:'task'`）的 `state.metadata.sessionId` 里——`chat_persist.py`（`_tool_sessions_by_msg`/`_pending_subtasks_by_msg` 配对）与 `batch_engine.py::discover_subtasks`（subtask part 与 tool part 按出现顺序配对）都改为从 tool part 取真实子会话 id，因此**自然语言委托**（只产出 tool:'task' part、没有 subtask part）同样能被发现追踪；`/command` 路由（`run_session_command`）也改为在阻塞的 `run_command` **之前**挂 `ensure_listener`（C-2），整轮对话与子代理轨迹都能实时落库。

### AI Agent Chat — Batch Tasks (M1.5)

Pick N files + 1 prompt → N isolated sessions, throttled to 3 concurrent. Children are real `ai_chat_sessions` rows with `batch_id` / `batch_seq` / `batch_input_file` set; the existing chat view handles them as-is, so clicking a child from the batch dashboard opens its full conversation thread. A batch carries an optional **`agent`** and **`model`** (`ai_chat_batches.agent`/`model`, both nullable; empty → OpenCode/global default) chosen in `CreateBatchDialog`; the worker passes them to `send_prompt_async`, and both are shown in the batch list/detail. Completion is detected from OpenCode's REST message state (`info.finish` terminal, i.e. ≠ `tool-calls`), not the transient `session.idle` event, plus a `STALL_TIMEOUT_SEC` no-progress watchdog — see `batch_engine._await_finished`/`_OpenCodeFacade.list_messages`.

*   **Worker**: `server/utils/batch_engine.py` — in-process `BatchWorker` (daemon thread + `ThreadPoolExecutor(3)`), started from `app.py` next to the existing schedulers under `WERKZEUG_RUN_MAIN`. Claims FIFO via `FOR UPDATE SKIP LOCKED`. 30-min per-child timeout. On Flask restart, audit resets orphaned `running` rows back to `pending`. For regular children it prepends an **input-file hint** (`uploads/<file>`) to the prompt (`_with_input_hint`) so the agent reads the upload instead of asking for a path; scan children keep their own preamble (`ai_scan_engine.assemble_prompt`). The worker now persists the **full conversation** for each child session (user prompt + every assistant message including tool-call bubbles) via `_content_from_parts` (maps OpenCode message parts to `text`/`tool_use` content types) and a rewritten `_persist_conversation` that reads all OpenCode messages and stores them in `ai_chat_messages`. **mem0 long-term memory** (see the AI Agent Chat memory paragraph above) is wired into every child session, not just interactive chat: `_run_one` prepends a `render_memory_block(search_memory(user_id, prompt))` block to the prompt before sending (mirrors `routes/ai_chat.py::send_message`'s injection exactly), and on a successful finish fires `_record_memory` — a background-thread `add_memory` call, deliberately NOT `utils/memory.py::extract_from_turn` (that function actively skips rows with a `batch_id`/`scan_task_id`, since it's scoped to real interactive sessions). Because `ai_scan_engine.run_task` also goes through `create_batch`/`_run_one` under the hood (scan children are just batch children with `scan_task_id` set), this one hook covers both batch tasks and scheduled AI scans — no separate wiring needed in `ai_scan_engine.py`. `continue`-mode turns (reusing an existing OpenCode session with full history) intentionally skip both injection and extraction — redundant given the session already has the context.
*   **REST**: `server/routes/ai_chat_batches.py` (POST/GET/DELETE/`retry-failed`/`append`/PATCH + staging upload), `server/routes/ai_chat_prompt_templates.py` (per-user CRUD). Both registered after `ai_chat_bp` and before `dynamic_bp` (which stays last). `POST /ai/chat/batches/<id>/append` (`append_to_batch`) appends staged files to an existing batch in any status; seq continues from the current max, `total` increments by N, status resets to `running`, and the worker picks up new children automatically. `PATCH /ai/chat/batches/<id>` (`update_batch_config`): edits `agent`/`model` on an existing batch (empty string → NULL = default, owner-only); the worker reads these fresh per run (`_fetch_batch_context`), so changes take effect on the next retry/reexecute/pending run.
*   **Frontend**: AI 助手 sidebar displays batch tasks as **collapsible `BatchGroup` entries** inline in the session list (no separate tab). Each group header shows status/progress/agent/model and offers 编辑(Agent/模型)/追加文件/重试失败/删除 actions; the「编辑」icon opens `EditBatchConfigDialog` to change agent/model (prefilled, clearable=默认). Clicking a child opens its full conversation thread with tool-call bubbles. `BatchListView`/`BatchDetailView` have been removed.

**Admin page** (`/admin/ai-batches`, `src/views/admin/AiBatchAdmin.vue`): cross-user, read-mostly monitoring surface separate from the owner-scoped sidebar view above. Backed by its own blueprint `server/routes/ai_batch_admin.py` (`/ai/chat/admin/batches`), gated by `admin.ai_chat_admin`, and — like all non-`dynamic_bp` blueprints — **registered before `dynamic_bp`** in `app.py`. Data layer is a separate set of `admin_*` read functions in `batch_repo` (`admin_list_batches`/`admin_get_batch_detail`/`admin_get_batch_owner`/`admin_get_child_messages`) rather than making the existing owner-scoped functions' `user_id` nullable — deliberately, so no function in the system silently becomes "read across all users" by omitting a filter. Its two write endpoints (retry-failed, reexecute) call `admin_get_batch_owner` first, then reuse the existing owner-filtered write functions (`reset_failed_to_pending`/`reexecute_child`) — so there's still no "unscoped write" function anywhere. Child messages are ordered by `seq` and capped at `MAX_ADMIN_MESSAGES` (500).
*   **Failure policy**: failed children are red-flagged and never abort the batch; `POST /ai/chat/batches/:id/retry-failed` resets failures to `pending` for the worker to pick up. `POST /ai/chat/batches/<id>/sessions/<sid>/reexecute` (`reexecute_child`): single-child re-execute, terminal states only (completed/failed); deletes that child's `ai_chat_messages` + clears `opencode_session_id`/preview/error + decrements done/failed counter + recomputes batch status → `running`; worker picks it up as `pending` and runs a fresh OpenCode session from scratch (unlike bulk `retry-failed` which keeps old context). No auto-retry.
*   **产出文件**（2026-08-14）: 管理员可在子任务行点「产出文件」打开独立 dialog，列出该子任务 AI 实际产出的文件（live scan workspace + LEFT JOIN `ai_chat_session_files.data_file_id`）、预览、下载、批量导入到 `data_files`（`uploaded_by` = 批任务原 owner，`operation_logs.operator` = 管理员）。导入白名单扩展为 `DB 白名单 ∪ live-scan 文件名集合`（admin 端独有；owner 端与对外 API 仍只 DB 白名单，行为不变）。新增端点：`/<batch_id>/sessions/<sid>/files`、`/files/preview`、`/files/download`、`/files/import`、`/changes`、`/files/diff`，全部走 `require_permission_sse('admin.ai_chat_admin')` 装饰器（`<a download>` 链接要靠 `?access_token=`）。owner 端 `/sessions/<sid>/files` refactor 为调 `utils/workspace_outputs.py::list_session_files`，行为等价。关键文件：`server/utils/workspace_outputs.py`、`server/routes/ai_batch_admin.py` 新增 6 endpts + `src/components/ai-chat/AdminBatchFiles.vue`、`server/auth.py::require_permission_sse`、`src/utils/request.ts::authParam`。

**对外 API**：批任务能力另有一套 API Key 鉴权的对外接口 `/api/v1/ai-batches/*`（`server/routes/open_api_batches.py`），刻意**不复用**内部 `ai_chat_batches.py` 的处理器 —— 内部返回体含 `opencode_session_id`/`workspace_path` 等实现细节，一旦对外就成了公开契约。对外层只吐约定字段（`batchId`/`status`/`total`/`done`/`failed`/`agent`/`model`/时间戳；结果为 `{name, status, output, error}`，`output` 从 `ai_chat_messages` 取最后一条 assistant 消息的全部 text 片段拼接，**不是** `last_message_preview`（那只存第一行），且**只在子任务 `status='completed'` 时返回**——`batch_engine` 运行期间会周期性落库对话，不设这道门就会把执行中的、以及超时失败前产出的**半截文本**当成结果吐给集成方）。端点：上传/创建/`PATCH`改配置/列表/详情/结果/删除/`retry-failed`/`append`/单条子会话 `reexecute`/`continue`（均已对外开放）。`append` 向已有批任务追加文件，**刻意不限终态**——追加语义就是「再加几个」，对已 `completed` 的批任务追加是合法用法；追加后 `total += N`、状态回到 `running`，已完成的子任务不重跑。`PATCH` 是**整体替换**语义（照抄内部 `update_batch_config` 的既有行为，`agent`/`model`/`provision_repo`/`provision_ref` 早就是这个语义）——省略的字段会被清空为默认值，不是局部 patch，调用方每次都要把想保留的字段一起带上；这条在对外文档（`docs/user-guide/integration/ai-batch-api.md` 4.2a）里明确提醒了，因为一旦对外它就是集成方会踩的坑，不像内部 UI 那样表单本身总是带全量字段。批任务的完成通知：`ai_chat_batches.callback_url`/`callback_secret`（可选，创建/`PATCH` 时设置）—— `batch_engine._recompute_batch_status` 收敛出终态（`completed`/`partial`/`failed`）时调 `_notify_callback`，复用 `webhook_engine._fire_single_webhook` 做 HMAC 签名 + 重试 + `webhook_logs` 落库，**不**新建 `webhook_rules` 行（那张表是为 `dynamic_data` 的 collection/trigger_event 语义设计的，批任务不是一行记录）。异步派发（裸 `threading.Thread(daemon=True)`，风格同 `utils/memory.py` 的 `add_memory`）——`_fire_single_webhook` 最坏阻塞 `timeout×(retries+1)`≈120s，而批任务 worker 只有 3 个并发槽位，同步调用会拖慢排队中的其它子任务。**没有"只通知一次"的去重**：`retry-failed`/`append`/`continue` 都能把已终态的批任务重新拉回 `running`，之后再次收敛到终态时会再触发一次回调，这是有意为之——每次收敛都是一次真实的"批任务完成"事件。`create` 与 `append` 共用 `_validate_files`（路径归属 + 文件存在性），两处各抄一份必然漂移、而每一条都是安全边界。隔离模型：`api_keys.owner_user_id` 绑定密钥创建者、`ai_chat_batches.api_key_id` 记录来源密钥，查/删/重试/追加一律带 `api_key_id` 条件，命中不到返回 404 而非 403；UI 建的批任务 `api_key_id IS NULL`，对 API 永远不可见。存量密钥 `owner_user_id` 为 NULL，被 `require_bound_key`（现在住在 `server/auth.py`，和 `api_key_required` 挨着，供本节及下一段的所有对外蓝图共用——原先只在这个文件里本地定义，多个蓝图各抄一份必然漂移，遂上移）挡在该接口之外（防止外部调用以某个人的名义跑 AI）。`files[].path` 必须经 `_validate_staged_path` 校验落在本密钥属主的 `batch-staging/<ownerUserId>/` 下 —— 内部接口不做这个校验（JWT 后天然受限），对外不补就是横向越权；注意这层是**用户**维度（同一用户名下多把密钥共享暂存空间），而批任务可见性是**密钥**维度，两者粒度不同。创建时还会 `os.path.isfile` 校验暂存文件**当前确实存在**（超 TTL 被清理的路径直接 400），`batch_engine._prepare_workspace` 在源文件缺失时抛 `FileNotFoundError` 让子任务落 `failed` —— 否则 AI 会在空工作区里"跑成功"、输出垃圾。请求体大小由**两道同源前置门**按 `Content-Length` 拦截（uploads 101 MB / JSON 1 MB，超限 413）：蓝图级 `before_request`（覆盖直连/Vite）+ `proxy.py::_reject_oversized`（覆盖生产入口，必须在 `rfile.read()` 之前），数值共用零依赖模块 `utils/upload_limits.py`，改一处两边一致。视图里的 20/100 MB 判断发生在 `f.read()` 之后，那时 body 已被读完；全应用刻意不设 `MAX_CONTENT_LENGTH`、代理层也不设全局上限（备份还原的 ZIP 大小无上界，全局值会打断还原）。用户文档：`docs/user-guide/integration/ai-batch-api.md`。

**AI 单会话**（`server/routes/open_api_ai_sessions.py`，`POST /v1/ai-sessions` 创建 + `GET /v1/ai-sessions/<id>` 查状态）：批任务的轻量兄弟——一句 prompt（可选带上几个文件）建一个独立会话，不按文件拆分子任务，也刻意不做 list/delete/continue/retry（见 `ai-session-api.md`「范围」一节）。执行完全复用批任务的 worker：`ai_chat_sessions` 行 `batch_id IS NULL` 但 `api_key_id` 非空即被 `batch_engine.py` 的同一个 dispatcher 认领（`_claim_pending_sessions`/`_restart_audit` 的 WHERE 子句已相应放宽），`_run_one` 在 `batch_id is None` 时从会话自己的列（而非父批任务行）取 prompt/agent/model——初始 prompt 复用 `continue_prompt` 列暂存、首次认领时读出即清空，跟"继续对话"复用同一列的既有模式一致。**可选文件**（`body.files`，形状同批任务）：不新建专属上传端点，复用 `POST /v1/ai-batches/uploads` 暂存 + `open_api_batches._validate_files`/`MAX_FILES_PER_BATCH` 校验；落在新列 `ai_chat_sessions.input_files`（JSONB 数组）里，`_prepare_workspace` 因此从"只接受单个字符串路径"泛化成"接受字符串或路径列表"（单字符串在内部包成一元素列表，行为对现有调用方——批任务子任务、扫描任务——逐字节不变），逐个拷进该会话自己的 `uploads/`；`_with_input_hint` 相应加了"1 个文件点名、2+ 个文件不点名"的措辞分支。响应里的 `files` 字段只回显文件名，不回显内部暂存路径。用户文档：`docs/user-guide/integration/ai-session-api.md`。

除批任务外还有三套更小的对外能力，各自独立成一个蓝图文件（沿用"一个外部能力一个文件"的既有惯例），都用 `api_key_required` + `require_bound_key`：
- **Row Actions 触发**（`server/routes/open_api_row_actions.py`，`POST /v1/collections/<collection>/<recordId>/row-actions/<actionId>/run`）：直接复用 `utils/row_action_engine.py::run_action`，和界面点「⋯」菜单里的按钮走同一条编排逻辑。API Key 没有角色，`action.roles` 白名单按**密钥属主本人的角色**（`SELECT role FROM users WHERE id = <ownerUserId>`）解析，等价于"密钥的操作权限=绑定用户本人"，和批任务 API 同一套归属模型。要求目标集合 `api_writable`（触发行操作本质是引发一次写入）。不提供行操作发现/列表端点——`actionId` 需调用方线下从配置该行操作的管理员那里获取，和 `scanTaskId`/`webhookRuleId` 的既有获取方式一致。**`error` 文案是中文**（`RowActionError.message` 原样透传），和 `open-api.md` 的英文惯例不同，这是本端点唯一的例外，文档里有显著标注。
- **AI 定时扫描任务 `run-now`**（`server/routes/open_api_scan_tasks.py`，`GET /v1/ai-scan-tasks[/​<id>]` + `POST /v1/ai-scan-tasks/<id>/run-now`）：内部 `routes/ai_scan_tasks.py::run_now` 完全是 `admin.ai_scan` JWT 权限门、且查询是全局不限归属的（`ai_scan_repo.get_task`/`list_tasks` 本来就没有 owner 过滤，管理员本该看到全部任务）；对外版本新增 `ai_scan_repo.get_task_for_owner`/`list_tasks_for_owner`（按 `owner_user_id` 过滤，不改现有无归属函数）。`run-now` 和内部路由一样同步调 `utils.ai_scan_engine.run_task`（认领 + 建批任务是同步的，真正的 AI 执行经批任务 worker 异步完成，所以不会长时间阻塞请求），响应里额外带一次重新查询后的 `claimedCount`/`lastError`，省调用方一次轮询。不检查 `task.enabled`——内部 `run-now` 本来就不检查，对外包装层不引入内部没有的新规则。error 文案沿用中文（和批任务 API 同一个"AI 相关对外端点"家族）。
- **Prompt 模板 + 长期记忆对外 CRUD**（`server/routes/open_api_prompt_templates.py` 的 `/v1/prompt-templates/*`，`server/routes/open_api_memories.py` 的 `/v1/memories/*`）：分别直接复用 `utils/prompt_template.py`（本来就以 `user_id` 为第一参数、SQL 层按 `user_id` 过滤）和 `utils/memory.py` 的既有函数，把 `g.current_user['userId']` 换成 API Key 的 `ownerUserId`。**记忆删除是安全关键点**：`utils/memory.py::delete_memory(memory_id)` 不按 `user_id` 过滤（直接调 mem0 的 `m.delete(memory_id=...)`），内部 `routes/ai.py::delete_my_memory` 靠"先 `list_memories(自己)` 建一个 owned-id 集合，删之前检查目标 id 在不在集合里"这道路由层闸门堵住 IDOR，对外端点**完整照抄**这道闸门，不能因为是新代码就漏掉。Prompt 模板的 error 文案沿用内部路由自己的英文文案（`name and content required` 等——那个内部路由本身就不是中文，不属于上面两条的"AI 中文家族"）。

Key files: `server/utils/batch_repo.py`, `server/utils/batch_engine.py`, `server/utils/prompt_template.py`, `server/routes/ai_chat_batches.py`, `server/routes/ai_chat_prompt_templates.py`, `src/types/aiChatBatch.ts`, `src/stores/aiChatBatches.ts`, `src/components/ai-chat/BatchGroup.vue`, `src/components/ai-chat/CreateBatchDialog.vue`, `src/components/ai-chat/EditBatchConfigDialog.vue`, `src/components/ai-chat/PromptTemplateManager.vue`. Design + plan: `docs/superpowers/specs/2026-05-31-ai-chat-batch-tasks-design.md`, `docs/superpowers/plans/2026-05-31-ai-chat-batch-tasks.md`. E2E: `e2e/ai-chat-batch.spec.ts`.

### AI Scheduled Tasks (定时 AI 数据流水线)

A **generic** config-driven paradigm (audit is one instance): on a schedule, scan a data page, hand each pending record (fields + attached documents) to an AI session with a prompt/skill, parse its structured JSON, and write results back into the record while flowing a **status field** 待处理 → 处理中 → 已处理/处理失败. See `docs/user-guide/ai/scan-tasks.md`.

*   **Reuses the AI batch engine**: each scan = one batch, each record = one child session (stamped `scan_task_id` + `source_record_id`); a completion hook in `batch_engine._run_one` calls `ai_scan_engine.on_child_finished` to parse + write back.
*   **Scheduler** `server/utils/ai_scan_scheduler.py` (APScheduler 1-min tick, per-task due/lock, startup orphan sweep, `WERKZEUG_RUN_MAIN` guard). **Engine** `server/utils/ai_scan_engine.py` (atomic `FOR UPDATE SKIP LOCKED` claim→处理中; context dir = `record.md` + `attachments/`; prompt = your template + auto-appended JSON contract from the field mapping; write-back via parameterized `jsonb_set`). **Repo/API** `server/utils/ai_scan_repo.py`, `server/routes/ai_scan_tasks.py` (CRUD + run-now, `@require_permission('admin.ai_scan')`).
*   **Config table** `ai_scan_tasks`; admin page `src/views/admin/AiScanTaskManager.vue` (`/admin/ai-scan-tasks`). Cross-table side effects are left to the skill via MCP; only same-row structured write-back is built in. Design + plan: `docs/superpowers/specs/2026-06-03-scheduled-ai-row-processor-design.md`, `docs/superpowers/plans/2026-06-03-scheduled-ai-row-processor.md`.

### Database Design

*   **`dynamic_data`**: The core table. `data` column holds all fields as JSONB. Has `branch_id` for version isolation. `search_text` (plain TEXT, `utils/search_text.py::compute_search_text`) is a precomputed concatenation of every "directly searchable" field's value (text/textarea/markdown/number/autoSequence/select/radio/date/datetime/autoTimestamp/compositeText), maintained by every write path (`routes/dynamic.py` create_item/update_item/batch_create_items, `routes/open_api.py` create/update) and indexed with a `pg_trgm` GIN index (`idx_dynamic_data_search_trgm`) so keyword search (`routes/dynamic.py::build_keyword_conditions`) can do `search_text ILIKE %s` instead of one `data->>field ILIKE` scan per field — the JSONB GIN index (`idx_dynamic_data_gin`, default `jsonb_ops`) does **not** accelerate `->>` text extraction, only containment ops (`@>`/`?`). Relation/reference/quoteSelect fields are excluded from `search_text` (they're matched via JOIN/EXISTS against the target record, not this row's own text). New/changed rows get `search_text` from the write path listed above; pre-existing rows (and rows from write paths that don't yet stamp it — `utils/etl_engine.py`, `utils/project_version.py`, `utils/trigger_engine.py`, `utils/version.py`, `utils/workflow_engine.py` all still do raw `INSERT INTO dynamic_data` without `search_text`, a known gap left for a follow-up) are backfilled by `init_db.py` (`_backfill_search_text`, batched, idempotent — re-running only touches rows still `search_text IS NULL`). Two supporting indexes, both built via `_create_concurrent_index` (shared helper, own autocommit connection since `CREATE INDEX CONCURRENTLY` can't run inside a transaction — and self-healing: it detects a same-named index left `indisvalid = false` by a previously interrupted/killed `CONCURRENTLY` build via `pg_index` and drops+rebuilds it, since plain `IF NOT EXISTS` only checks the name and would otherwise skip a broken index forever — verified by actually killing a live `CONCURRENTLY` build mid-flight and confirming rebuild): `idx_dynamic_data_search_trgm` (the trigram GIN index used for `ILIKE`) and `idx_dynamic_data_search_text_pending` (a partial btree index on `(collection) WHERE search_text IS NULL` — without it, `_backfill_search_text` re-scans every row of every already-fully-backfilled collection on every `init_db.py` re-run just to confirm nothing's left; Postgres auto-maintains partial indexes as rows stop matching the predicate, so this shrinks to near-empty and stays cheap, and as a side effect it also catches rows from the un-migrated write paths above whenever `init_db.py` is next re-run).
*   **`field_indexes`**: Tracks admin-requested per-field expression indexes on `dynamic_data` (`FieldConfig.indexed: boolean`, editable per-field in the field config editor even on pages that already have data — it's the one field attribute exempt from the "existing data locks field edits" rule, since it doesn't touch stored data shape). `utils/field_indexes.py::sync_field_indexes` (called from `routes/page_configs.py` create/update) diffs the desired indexed-field set against this table and inserts `status='pending'` / marks `status='dropping'` rows; `utils/field_index_scheduler.py` (30s poll) asynchronously runs `CREATE`/`DROP INDEX CONCURRENTLY` on a `(data->>'field')` partial index scoped `WHERE collection = '<collection>'`, since building on a huge table can take minutes and can't block the page-config save request. Index names are `idx_dyn_fld_<md5(collection+field)[:16]>` (deterministic, ASCII-safe regardless of collection/field name characters). `GET /pageConfigs/<id>/field-indexes` exposes live status (pending/building/ready/failed/dropping) polled by the field config editor UI. **Important**: psycopg2's `%s` substitution is client-side (mogrify), so `data->>%s` queries already send the field name as a SQL literal — no query-construction changes were needed in `routes/dynamic.py`/`utils/mongo_query.py` for the resulting index to be picked up by the planner; confirmed via `EXPLAIN`. Indexed fields are also added to the main list endpoint's sort whitelist (`routes/dynamic.py::list_items`, normally hardcoded to `createdAt`/`updatedAt`/`id` to prevent unindexed full-table sorts).
*   **`data_relations`**: Stores M:N relationships. Queried by `(collection, record_id, field_name)`.
*   **`page_configs`**: The schema definition. `fields` JSONB column drives the entire UI.
*   **`menus`**: Tree structure with `roles` JSONB for RBAC. Three types: `workspace` (level 1), `project` (level 2), `data` (level 3).
*   **`users`**: Accounts with `role` (admin / developer / guest).
*   **`project_versions`**: Project-level branches/snapshots. Tracks status (`active`, `merged`, `archived`), parent version, and lock state.
*   **`project_dependencies`**: Cross-project dependency declarations. Three relation types: `track-main`, `read-write`, `read-only`.
*   **`project_dependency_relations`**: Records which data relations are involved in each dependency.
*   **`webhook_rules`**: Event-triggered webhooks. Supports `create`, `update`, `delete`, `merge` events with HMAC-SHA256 signatures.
*   **`webhook_logs`**: Webhook execution history with retry tracking.
*   **`notifications`**: User notifications including dependency alerts (`dependencyBroken`, `dependencyWarning`, `dependencyResolved`).
*   **`column_views`**: Per-page saved column-view configurations (visibility, order, width, filters). Frontend reloads these on keep-alive reactivation to prevent cross-page view leak (see commit `932b2d6`).
*   **`ai_chat_sessions`** / **`ai_chat_messages`**: AI Agent chat sessions (opaque `session_token`, `workspace_path`, `opencode_session_id`, status) and their messages (`content` JSONB of typed parts). See the AI Agent Chat section above.
*   Other tables: `export_scripts`, `validation_scripts`, `etl_tasks`, `etl_logs`, `api_keys`, `operation_logs`, `backups`, `backup_settings`, `ai_settings`, `collection_versions`, `version_snapshots`, `dashboards`, `record_comments`, `trigger_rules`, `system_config`, `home_widgets`.

### Backend Structure

*   `app.py`: Registers ~39 blueprints (auth, users, menus, page_configs, relations, operation_logs, backups, export_scripts, api_keys, open_api, open_api_batches, open_api_row_actions, open_api_scan_tasks, open_api_prompt_templates, open_api_memories, validation_scripts, etl_tasks, relation_graph, query, comments, timeline, dashboards, notifications, trigger_rules, ai, project_versions, cross_project_deps, webhook, menu_export, system_config, home_widgets, column_views, ai_chat, ai_chat_prompt_templates, ai_chat_batches, ai_scan_tasks, data_files, roles, workflows, dynamic). **Order matters**: `dynamic_bp` (catch-all `/<collection>`) must be registered last to avoid shadowing specific routes; the AI-chat/scan and admin blueprints (`ai_chat`, `ai_scan_tasks`, `roles`, `workflows`, …) are all registered before it. All `/v1/*`-prefixed open-API blueprints are registered in one cluster right after `auth`/`api_keys` — new external-API additions belong in that cluster, before `dynamic_bp`.
*   Four background components start in `app.py` (guarded by `WERKZEUG_RUN_MAIN` to avoid double-start in Flask reloader): `start_backup_scheduler` (scheduled backups), `start_dependency_scheduler` (hourly cross-project dependency revalidation), the in-process AI batch worker (`get_worker().start()`), and `start_scan_scheduler` (scheduled AI scan tasks).
*   `routes/dynamic.py`: The generic CRUD handler. Supports pagination (`page`, `pageSize`, `all=true`), filtering (`q` for MongoDB-style query, `keyword` for full-text search). Validates primary key uniqueness and manages relations.
*   `routes/project_versions.py`: Project-level branch management. Create, merge, delete, lock/unlock branches.
*   `routes/cross_project_dependencies.py`: Dependency declaration CRUD, validation, merge dependency check, branch delete protection.
*   `routes/webhooks.py`: Webhook rule management, test execution, log retrieval.
*   `utils/script_runner.py`: Sandboxed Python execution for validation/export scripts.
*   `auth.py`: JWT decorators — `login_required`, `write_required` (blocks guest), `require_permission('admin.x')` (RBAC capability gate; replaced the old `admin_required`), `api_key_required`. See `utils/permissions.py` + `utils/rbac_guard.py` and `docs/user-guide/admin/roles-rbac.md`.
*   `db.py`: `psycopg2.pool.ThreadedConnectionPool` (2–20 connections, thread-safe). Use the `get_db()` context manager. Imported as `from db import get_db`. The production backend runs under waitress with a bounded thread pool (`BACKEND_THREADS`, default 8) kept below `maxconn`.

**Reserved collection paths** (cannot be used as dynamic data collection names; authoritative list lives in `RESERVED` at `server/routes/dynamic.py:18`): `menus`, `pageConfigs`, `relations`, `auth`, `users`, `roles`, `operationLogs`, `backups`, `exportScripts`, `apiKeys`, `validationScripts`, `etlTasks`, `relation-graph`, `query`, `comments`, `timeline`, `dashboards`, `notifications`, `triggerRules`, `ai`, `versions`, `project-versions`, `webhook`, `dependencies`, `system-config`, `home-widgets`, `data-files`, `ai-scan-tasks`, `favicon.ico`.

### Frontend Structure

*   `src/stores/pageConfig.ts`: The central store. Contains critical logic for:
    *   Resolving relation/reference/quote field imports.
    *   Generating auto-sequence values.
    *   Batch data operations.
*   `src/stores/menu.ts`: Menu tree with shallowRef caching + role-based filtering.
*   `src/stores/auth.ts`: JWT token management. localStorage keys prefixed `check-manage:`.
*   `src/components/dynamic-form/controls/`: Field control components. Mapped by `controlType`.
*   `src/api/`: Axios-based API layer. Base URL `/api`, 30s timeout. Request interceptor injects Bearer token.
*   `src/views/admin/hub/settingsCatalog.ts`: 设置中心的**唯一真源**。22 个设置功能按 7 个视觉分组组织，每条带 `id`（路由末段）/`label`/`perm`（`admin.*` 能力键）/`component`（懒加载）。侧边栏渲染（`src/components/layout/SettingsSideMenu.vue`）、路由生成（`src/router/settingsRoutes.ts`）、权限判定（`auth` store 的 `hasRoutePermission`）全部从它派生 —— 新增一个设置功能只需在这里加一条。分组只是视觉分组，**不参与路由**：每个功能都是独立路由 `/admin/<id>`，一次点击到达。进入设置路由时 `AppLayout` 按 `meta.shell === 'settings'` 用设置侧边栏替换业务侧边栏。老路径兼容见 `LEGACY_PATH_ALIASES` 与 `buildSettingsRedirects`。

### Excel/JSON Data Import (Large-File Safe)

Importing an Excel/JSON file (`DynamicPage.vue`'s 导入数据 action, also used by `ImportTab.vue`) is split into a **parse phase** (in a Web Worker) and an **upload phase** (main thread, batched HTTP), both driving the same 导入数据 dialog's progress bar so large files never look "stuck":

*   **Parse phase** — `src/workers/excelImportWorker.ts` reads the `File` (`arrayBuffer()`/`text()`) and calls the pure, chunkable functions in `src/utils/excelParseCore.ts` (`readWorkbookMeta`/`processWorkbookRowRange` for xlsx, `readJsonMeta`/`processJsonItemRange` for JSON — `XLSX.read`/`sheet_to_json`/`JSON.parse` are monolithic SheetJS/JSON calls and can't be chunked, but the per-row/per-item "raw data → record" mapping loop after them can). The worker processes `CHUNK_SIZE = 20000` rows at a time and `postMessage`s each chunk immediately (instead of the whole array at the end) — for a 1M-row file this cut the one-time `postMessage` structured-clone stall from ~2.1s to ~0.6s (measured with a `setInterval` tick probe; a single monolithic postMessage of ~1M records blocks the main thread for the whole deserialize). `src/utils/excel.ts`'s `parseImportFile`/`parseJsonImportFile` accumulate the chunks and report `(processed, total)` via an optional `onProgress` callback, which `DynamicPage.vue` uses to show "正在解析文件... X / Y 行" before switching to the upload phase's "正在导入... X / Y". `fields` is JSON-round-tripped (`JSON.parse(JSON.stringify(fields))`) before `postMessage` because it comes from a Pinia store and Vue's reactivity Proxies aren't structured-cloneable (throws `DataCloneError` otherwise).
*   **Upload phase** — `src/utils/importPageRecords.ts` batches records into `/​<collection>/batch-create` calls: `BATCH_SIZE = 1000`, run at `CONCURRENCY = 3` (a small worker-pool pattern — each concurrent worker pulls the next batch index from a shared cursor, so batches stay correctly ordered for sequence-value assignment regardless of completion order). Chosen conservatively because the backend does an unindexed per-record primary-key uniqueness check inside `batch-create` when the page has PK fields (`routes/dynamic.py::check_primary_key_unique`, one query per record) — a much larger batch size or higher concurrency risks slow/timing-out requests on PK-bearing pages; 3 concurrent requests stays well under waitress's default `BACKEND_THREADS=8` and the DB pool's `maxconn=20`.
*   **Measured impact** (1,000,000-row file, same machine): total wall time ~651s → ~411s after raising batch size/concurrency; the parse→upload handoff stall (one-time, not recurring — verified with a tick probe across both runs) dropped from 2094.9ms → 609.4ms after chunking the worker's response.

### API Proxy

Vite proxies `/api` to backend port 3002 **with path rewrite**: frontend calls `/api/menus` → backend receives `/menus`. This is why backend routes don't have an `/api` prefix.

### User Roles (Customizable RBAC)

Roles are **data-driven and customizable** (see `docs/user-guide/admin/roles-rbac.md`). Permissions are controlled at three granularities: **admin-feature toggles** (`admin.*` capability keys), **per-data-page CRUD**, and **menu visibility** (`menus.roles`).

*   Built-in seeds: **admin** (permanent superuser — all permissions, undeletable), **developer** (read/write data, no admin features), **guest** (read-only). `developer`/`guest` are editable; custom roles can be added.
*   Backend is authoritative: `@require_permission('admin.x')` (replaced `admin_required`) + `require_page_action()` in `routes/dynamic.py`/`relations.py`. Resolution + cache in `server/utils/permissions.py`; catalog in `PERMISSION_CATALOG`.
*   Frontend gating (UX only): `auth` store `isSuperuser`/`can()`/`canPage()`; `/admin/roles` role manager. JWT carries only the role slug — permission edits take effect on next `/auth/me` without re-login.
*   ⚠️ **Capability ≠ menu visibility**: granting an `admin.*` capability lets a role *access* a route, but the sidebar link only shows if the role slug is in that menu's `menus.roles`.

### Menu Structure (Standard 3-Layer Hierarchy)

Menus follow a strict 3-level hierarchy enforced by `menuType` field:

| Level | menuType | Description | Example |
|-------|----------|-------------|---------|
| 1 | `workspace` | Top-level container, no `page_id` | "测试工作空间" |
| 2 | `project` | Project node, children are data pages | "项目A" |
| 3 | `data` | Data page, has `page_id` pointing to `page_configs` | "测试订单" |

Projects can be direct children of workspace (level 2) OR standalone level-1 menus with `menuType='project'`. Data pages must have valid `page_id` and `parent_id` pointing to a project.

## Development Workflow

### ⚠️ Documentation Sync (MANDATORY)

**Every user-facing feature change MUST update the user guide in the same PR.** When you add, change, or remove a feature that a user interacts with, update the matching doc under `docs/user-guide/` (organized by feature: `getting-started/ data/ admin/ integration/ ai/`, English-slug filenames). If no doc exists for the feature, create one in the right subfolder and link it from `docs/user-guide/README.md` (the index/TOC). Where a flow has UI, include a real page screenshot under `docs/user-guide/_images/`. Treat the doc update as part of "done" — a feature change without its user-guide update is incomplete.

**Open API surface changes (`/api/v1/*`) additionally require syncing the two hand-authored HTML reference pages, not just the Markdown source.** Any change to an external-API endpoint's shape (new/removed/renamed endpoint, changed request/response field, changed error code) must be reflected in:
1. The relevant Markdown doc(s) — `open-api.md` for data-collection endpoints; `ai-batch-api.md`/`ai-session-api.md`/`data/row-actions.md` §11/`ai/scan-tasks.md` §10/`ai/batch-tasks.md`'s Prompt 模板 section/`ai/long-term-memory.md`'s 对外 API section for the six AI-related capability families.
2. The matching section of whichever styled HTML reference page covers that endpoint: `docs/user-guide/integration/open-api-reference.html` (data-collection endpoints) or `docs/user-guide/integration/ai-api-reference.html` (the six AI capability families) — these are curated, hand-maintained companions to the Markdown docs, not generated from them, so nothing keeps them in sync automatically.
3. **The public copy**: after editing either HTML reference page under `docs/user-guide/integration/`, copy it to `public/docs/` (`cp docs/user-guide/integration/<file>.html public/docs/<file>.html`) — that's the copy Vite/proxy.py actually serves at `/docs/<file>.html`; there is no build step that does this copy for you, so editing only the source copy silently ships stale docs.

### ⚠️ Playwright Verification (MANDATORY for UI / full-stack changes)

**Any change that touches a frontend page OR exercises frontend↔backend interaction MUST be verified live with Playwright before it's considered done** — unit tests and `vue-tsc` are necessary but NOT sufficient. This covers: new/changed Vue views or components, panels, dialogs, status rendering, and any feature whose correctness depends on the browser calling the backend (create/list/poll flows, SSE, settings pages, file panels, etc.).

Procedure:
1. Ensure services are running on the branch under test (`npm run dev:all` or restart the backend with `python app.py` so it loads the changed code — the Werkzeug/waitress process loads modules at startup, so backend changes need a restart). Frontend changes are picked up by Vite HMR.
2. Drive the real flow through the browser with the Playwright MCP tools (navigate, log in `admin`/`admin123`, perform the user actions), and assert on the rendered result — not just a 200 response.
3. Where the UI reflects backend state, cross-check the DB / on-disk artifact (e.g. a session's `opencode.json`, a `git status`, a batch row) so you're verifying the real wiring, not a stale cache.
4. Capture a screenshot of the key result into `.playwright-mcp/` as evidence and state plainly in the PR/summary what was verified.

Backend-only changes with no UI/interaction surface (pure utils, scripts, migrations) don't require Playwright — unit tests suffice. When in doubt (does a user see this?), verify with Playwright.

### ⚠️ proxy.py Verification (MANDATORY for SSE / streaming / new proxied routes)

**The Vite dev proxy (`:5173`) is NOT production.** Production serves the built `dist/` and proxies `/api/*` through `server/proxy.py` (`:8080`, reverse proxy in front of a waitress backend). Vite proxies SSE/streaming natively, so **a feature can pass all Playwright/Vite verification and still be broken in production** through `proxy.py`. This is a real, recurring gap.

**Any change involving SSE, chunked/streaming responses, a NEW `/api` route shape, or non-standard request headers MUST also be verified through `proxy.py` (`:8080`) before it's considered done** — not just via Vite.

The specific trap: `proxy.py._is_sse()` is an **explicit allowlist** deciding which responses stream vs. buffer. An SSE route not matched there is read to completion (`_relay_buffered`) and **hangs forever** in production. Likewise `_proxy_to_backend` forwards only an **allowlisted set of request headers** (`Content-Type, Authorization, X-API-Key, X-Visitor-Id, Accept`) — a new custom header a route depends on will be silently dropped. **When you add an SSE endpoint, you MUST update `_is_sse()`; when a route needs a new request header, you MUST add it to the forward list.**

Procedure:
1. `npm run build` (produces `dist/`), then `cd server && python proxy.py` (starts its own backend on `:3001` + serves `:8080`).
2. Exercise the real flow through `:8080` (Playwright, or `curl -N` for SSE). For SSE, confirm the stream **arrives incrementally** — e.g. `curl -sN --max-time 4 http://localhost:8080/api/.../events...` returns `event: ...` frames within the window; an **empty** result means the proxy buffered it (broken). Trigger a live event and confirm it arrives through `:8080` in real time.
3. State plainly in the summary that the flow was verified through `proxy.py`, not only Vite.

The SSE routes that must stay matched by `_is_sse()`: `/api/ai/chat/sessions/<id>/events`, `/api/kefu/sessions/<sid>/events`, `/api/kefu/sessions/<sid>/human-events`, `/api/admin/kefu/events`.

**The same trap applies to request-body limits.** `_proxy_to_backend` does `self.rfile.read(content_length)` — a synchronous, whole-body read into the *proxy* process — **before** Flask sees anything, so a Flask-side `before_request` size check is invisible in production. The AI-batch open API's limit therefore lives in **two** places reading one shared, zero-dependency module `server/utils/upload_limits.py` (`body_limit_for_path`): `routes/open_api_batches.py` (`before_request`, covers direct/Vite) and `proxy.py` (`_reject_oversized`, rejects before the read, replies 413 + `Connection: close` and closes the connection — bounded 64 KB/0.5 s lingering drain so the client can still read the response, never the declared length). `body_limit_for_path` returns `None` (= unlimited) for every other path **by design**: `/api/backups/upload-restore` receives backup ZIPs containing `vector_store/` + `data_files/`, whose size is unbounded — a global body cap at either layer would break disaster recovery. If you add another externally-exposed upload endpoint, add its limit there, never globally.

### Adding a New Business Entity (e.g., "Products")

**Do NOT** create a new Vue file or SQL table.

1.  Insert a row into `page_configs`:
    ```sql
    INSERT INTO page_configs (id, name, fields)
    VALUES ('page-products', '产品管理', '[{"fieldName": "name", "label": "产品名称", "controlType": "text", ...}]');
    ```
2.  Insert a row into `menus`:
    ```sql
    INSERT INTO menus (id, name, page_id, path, parent_id, "order")
    VALUES ('menu-products', '产品管理', 'page-products', '/products', NULL, 1);
    ```
3.  Done. The system creates the route `/products`, the API `GET/POST /products`, and the UI.

### Adding a New Field Control Type

1.  **Frontend**: Create `src/components/dynamic-form/controls/NewControl.vue`.
2.  Register it in `DynamicForm.vue` and `DataTable.vue` column rendering.
3.  Update `ControlType` in `src/types/field.ts` and re-export from `src/types/index.ts`.
4.  **Backend**: If validation is needed, update `routes/dynamic.py` validation logic.

### Script Runner Sandbox

Validation and export scripts execute in a sandboxed Python environment (`utils/script_runner.py`):
*   **Timeout**: 60s for row-level scripts, 300s for collection-level scripts.
*   **Forbidden**: `import`, `eval`, `exec`, `open`, `getattr`, `type`, etc.
*   **Pre-injected modules**: `json`, `csv`, `io`, `re`, `math`, `collections`, `xml.etree.ElementTree`, `datetime`, `pandas` (as `pd`), `numpy` (as `np`).
*   **No `print()`** — scripts communicate via return values.

### Creating Test Dependency Data

To test cross-project dependency features, run the test data creation script:

```bash
cd server && python create_test_dependency_data.py
```

This creates:
*   Workspace "测试工作空间" with 3 test projects (A, B, C)
*   Versions for each project (merged and active branches)
*   Four dependency declarations demonstrating all three types
*   Test data pages with relation fields (订单 → 产品 → 客户)
*   Data relations between collections across projects

## Key Technical Details

*   **Authentication**: JWT in `Authorization: Bearer <token>` header. Middleware decorators in `server/utils/auth.py`.
*   **Default Credentials**: admin / admin123 (set in `server/seed_data.py`).
*   **Database Config**: `server/config.py` — PostgreSQL on localhost:5432, database `casemanage`.
*   **TypeScript**: Strict mode enabled (`noUnusedLocals`, `noUnusedParameters`). Path alias `@/` → `src/`.
*   **SCSS**: Global variables via `src/assets/styles/variables.scss` (auto-imported in all SCSS).
*   **Production**: `start.sh` builds frontend, starts backend + reverse proxy on port 8080. The backend is served by **waitress** (bounded thread pool, `BACKEND_THREADS` env, default 8 — kept below the DB pool's `maxconn=20`); the dev server (`npm run dev`) stays on Werkzeug with `threaded=True`.

## Testing Patterns

Frontend tests: `src/**/__tests__/**/*.test.ts` with jsdom environment. Setup file: `src/test/setup.ts` (provides localStorage mock).

### ResizeObserver Polyfill
jsdom environment lacks ResizeObserver. Add this polyfill in test files when components use Element Plus or resize-dependent features:
```typescript
beforeAll(() => {
  globalThis.ResizeObserver = class ResizeObserver {
    observe() {}
    unobserve() {}
    disconnect() {}
  } as any
})
```

### Element Plus Component Stubs
For isolated component testing, stub Element Plus components with minimal implementations:
```typescript
const stubs = {
  'el-checkbox': {
    template: `<input type="checkbox" :checked="modelValue" @change="$emit('change', $event.target.checked)" />`,
    props: ['modelValue'],
    emits: ['change'],
  },
  'el-button': {
    template: `<button @click="$emit('click')"><slot /></button>`,
    emits: ['click'],
  },
}
```

### Deep Equality Assertions
Use `toStrictEqual` for object comparisons (not `toBe`):
```typescript
// ❌ Wrong - fails for objects
expect(state.sourceVersion).toBe(version)
// ✅ Correct - deep equality
expect(state.sourceVersion).toStrictEqual(version)
```

## Composables Pattern

Use Vue 3 composables for reusable state management. Key patterns:
- Return reactive state and computed properties
- Provide action functions that modify state
- Use `reactive()` for complex state objects
- Use `Set`/`Map` for collection state (more intuitive than arrays for toggle operations)
- Distinguish between data existence (`hasDiff`) and user action state (`hasSelection`)