...
| **状态徽标** | `statusBadge` | string | 只读，图标+颜色展示离散阶段；表单里不可编辑，只能由第三方系统（Open API）异步回写；可配置轮询间隔与超时兜底（超时自动写成失败态） |

#### 5.3.1 字段索引加速（大数据量场景）

标量类字段（文本/数值/单选/日期/自增序列/自动时间戳/组合文本/状态徽标/复选框）在字段编辑弹窗中有一个「加速筛选/排序」开关。勾选并保存后，后台会异步为该字段建立数据库索引，用于加快：

- 按该字段筛选（查询条件、看板分组等）
- 按该字段在数据表格中排序（未勾选的字段不允许排序，避免无索引全表排序拖慢大数据量页面）

建索引在后台任务里跑（不阻塞保存），字段列表和编辑弹窗里会显示实时状态：**待建 → 构建中 → 已就绪**（数据量大的页面可能需要几分钟）；失败会显示具体错误。取消勾选后索引会在后台自动清理。这个开关是唯一一个允许在「页面已有数据」时修改的字段属性——它只影响查询速度，不改变已存数据的形状。
