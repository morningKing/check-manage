...
| 400 | 上传：未提供文件 / 文件名无效 / 单文件超 20 MB / 累计超 100 MB。创建：`name`/`prompt` 缺失、`prompt` 超长、`files` 为空/超过 50 个/字段缺失、`files[].path` 未通过归属校验。列表：`page`/`pageSize` 不是整数 |
...
- 每把 API Key 只能看到、查询、重试、删除**它自己创建的**批任务；用另一把密钥（哪怕同一个用户名下）调用同样接口，看到的列表也不会包含对方的批任务，查/删/重试对方的 `batchId` 一律返回 404。
- **界面里手工创建的批任务对本 API 完全不可见**——UI 创建的批任务没有关联的来源密钥，反过来 API 创建的批任务在 UI 侧的会话列表里可以正常看到完整对话（因为底层复用同一套批任务引擎），只是不能反向从 UI 用密钥去操作。
- 上传接口 `/uploads` 返回的 `files[].path` 也和密钥的所属用户绑定，创建批任务时会校验该路径确实落在本密钥所属用户的暂存目录下，跨密钥/跨用户传路径会被拒绝（400）。
