...
- **变更文件**：基于工作区内 git 仓库的 `git status`，展示新增/修改/删除的文件及其 diff。`uploads/`、`outputs/` 被忽略，不在此面板出现。该面板在每轮对话结束后自动刷新；若助手写出的文件未被自动捕获，可点击 🔄 手动重新扫描。变更数量上限为 500，超出时**优先保留新增和修改的文件，剩余名额才显示删除的文件**（删除项通常信息量较低）。
