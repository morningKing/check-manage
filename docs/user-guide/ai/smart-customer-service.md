...
**管理端控制台（已交付）**：设置中心 → 集成对接 → 智能客服 → 选实例 → 「人工接管」tab。左右主从：
- **左·会话队列**：过滤「全部 / 待人工 / 接管中」；每项显状态徽标（待人工=橙 / 接管中=蓝 / AI=灰）、访客 id、最后消息预览、接管人、时间；约 5s 自动刷新，也可「🔄」手动刷新。
- **右·对话 + 操作**：点队列某会话看完整对话（访客右 / 客服左；人工消息带「人工」徽标）；未接管时点「接管」→ 变「释放」且回复框启用；输入人工回复发送即写入该会话（访客侧轮询收到）；点「释放」回到 AI。回复框**仅接管中可用**。
