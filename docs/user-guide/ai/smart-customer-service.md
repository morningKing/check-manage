...
支持把某个客服会话从 AI 切到**人工**：访客发起转人工 → 管理员接管（接管期间访客消息**不再派给 AI**，改由人工回复）→ 释放（回到 AI）。当前**后端接口 + 访客侧 + 管理端控制台均已交付**；实时推送（SSE 合并）属后续，本阶段访客与控制台均靠**轮询**（约 3–5s）刷新。

**访客侧（已交付）**：对话页顶栏有「🙋 转人工」按钮，点击后 → 顶部出现提示「已为你转接人工客服，请稍候…」、状态变「人工客服接入中」、按钮隐藏；此后每约 3 秒**轮询**拉取人工回复并显示为客服气泡。接管期间访客发消息照常入库、不触发 AI（也不卡「正在输入」）。管理员**释放**后回到 AI。
