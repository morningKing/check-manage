...
2. 顶部导航进入 **设置中心 → 智能客服**（`/admin/kefu`，独立路由，不再是与 Open API、Webhook 并列的 tab）。
...
在实例下拉框旁的按钮可直接管理实例，无需手写 API：

- **+ 新建客服**：打开弹窗，填写字段后保存，列表自动选中新实例。
- **编辑客服**（选中实例后出现）：修改字段后保存。
- **删除客服**（选中实例后出现）：二次确认后删除，**会级联删除该客服的所有热问，不可恢复**。

弹窗字段：

| 字段 | 说明 |
|---|---|
| **标识 slug** | 公开访问路径 `/kefu/<slug>`。小写字母/数字/连字符。**创建后不可改**（编辑时只读）。 |
| **名称** | 客服显示名（必填），也用于访客页头像首字。 |
| **启用** | 关闭后访客建会话返回 403。 |
| **欢迎语** | 访客打开对话时的首条客服气泡。 |
| **Agent** | OpenCode 命名 Agent，每次发消息透传；留空=默认 Agent。下拉复用 AI 助手的 Agent 列表。 |
| **模型** | OpenCode 模型（`<provider>/<model>`），写入会话并透传；留空=全局默认模型。 |
| **系统提示词** | 客服人设，运行时与固定护栏拼接后写入会话上下文（护栏始终在前生效）。 |
| **品牌 logo** | logo 图片 URL，存入 `branding.logo`，作访客页/头像用。 |

> 提示气泡、自助区块、热问在选中实例后于页面内联编辑（见 9.2.2、9.5）。
...
![访客页桌面端](../_images/kefu-page-desktop.png)

访客页采用**居中悬浮卡片**布局：浅色页面背景上居中一张大圆角、带柔和阴影的白色卡片装载整个客服界面。视觉基于一套**仅作用于本页**的柔和品牌蓝强调色（`--kefu-accent`，不影响后台其它页面的主题色）：

- **顶部栏**：白底克制，左侧客服头像（`branding.logo` 或客服名首字彩色圆标）+ 客服名 + 绿点「● 在线」。
- **消息区**：见下「消息气泡外观」。访客气泡为**品牌蓝**，客服气泡为白色卡片。
- **组合输入框**：一体化圆角容器——左侧 📎 附件按钮、中间自增高输入框、右侧纸飞机圆形发送按钮；聚焦时容器整体高亮描边。
- **加载/错误态**：加载时显示卡片骨架占位；实例不可用时卡片内显示友好提示。

#### 桌面端布局（≥992px）

卡片内呈**两栏布局**：

- **左栏 — AI 对话区**：白底顶部栏（头像 + 客服名 + 在线态）；欢迎语（以客服气泡形式、带头像展示）下方展示来自 `guided_questions` 的可点击提示气泡（浅蓝 pill）；历史消息流（见下「消息气泡外观」）；底部组合输入框。
- **右栏 — 持久自助服务区块列**（320px，固定展开）：按 `panel_blocks` 配置顺序从上到下渲染所有已启用区块，每块为独立小卡片（白底 / 描边 / 圆角）：快捷入口 / 热点问题 / 公告 / 联系方式。

> 右栏**仅在至少有一个已启用区块时**渲染；否则为纯对话布局，顶部栏也不显示「🗂 自助服务」按钮。

#### 移动端布局（<992px）

![访客页移动端抽屉](../_images/kefu-page-mobile.png)

卡片铺满视口（无圆角/阴影）；右侧服务列隐藏，顶部栏出现「🗂 自助服务」按钮（同样仅在有已启用区块时显示）。点击按钮弹出从右侧滑入的抽屉，内容与桌面端右栏相同（同样的卡片化区块）。

#### 消息气泡外观

对话区消息采用**双向气泡**布局，区分访客与客服（见上方整体外观截图）：

- **访客消息**：右对齐，**品牌蓝**气泡（白色文字），无头像。附件按类型渲染：**图片**（`.png/.jpg/.jpeg/.gif`）在气泡内**内联显示**缩略图（点击在新标签打开原图）；**其他文件**显示为 `📎 文件名` **下载链接**。图片/文件经 `GET /kefu/sessions/<sid>/files/<name>?visitor_id=`（归属校验 + `safe_resolve` 防穿越）取回。**管理端「人工接管」控制台的对话区同样渲染**访客发送的图片/文件（用会话 `visitor_id` 复用同一端点）。
- **客服消息**：左对齐，白色卡片气泡（带浅色描边），左侧圆形头像（优先使用 `branding.logo` 图片，否则取客服名称首字生成的彩色圆标）；内容 Markdown 渲染。
- **时间戳**：每条已入库消息在气泡下方显示 `HH:mm` 发送时间；本地乐观消息（尚未落库）不显示时间。
- **正在输入**：客服回复期间，对话区显示左侧三点动画气泡（对屏幕阅读器暴露 `role="status"`）。

> 品牌蓝强调色仅作用于访客页（`--kefu-accent`），不改动后台其它页面的主题主色；其余配色基于 Element Plus 主题变量。
...
## 10. 人工接管（Phase 2 · 后端 + 访客侧 + 管理端控制台，已交付）

支持把某个客服会话从 AI 切到**人工**：访客发起转人工 → 管理员接管（接管期间访客消息**不再派给 AI**，改由人工回复）→ 释放（回到 AI）。当前**后端接口 + 访客侧（SSE 实时）+ 管理端控制台均已交付**；**访客侧已升级为 SSE 实时**（接管/人工回复/释放即时到达）；管理端控制台实时（订阅同一总线）属后续，其 5s 轮询暂留。

**访客侧（已交付 · SSE 实时）**：对话页顶栏有「🙋 转人工」按钮，点击后 → 顶部出现提示「已为你转接人工客服，请稍候…」、状态变「人工客服接入中」、按钮隐藏。访客通过一条**独立的 SSE「人工事件」通道**（`/kefu/sessions/<sid>/human-events`）**实时**接收接管/人工回复/释放事件——人工回复即时显示为客服气泡（不再 3s 轮询）。接管期间访客发消息照常入库、不触发 AI（也不卡「正在输入」）。管理员**释放**后，访客页**即时复原**：提示条消失、状态回「在线」、「🙋 转人工」按钮重新出现（可再次转人工）。

> 实现：进程内事件总线（`server/utils/kefu_event_bus.py`）在管理端接管/回复/释放时按会话发布信号，访客 SSE 连接消费后拉取最新历史；EventSource 自动重连，重连即补拉，事件丢失可自愈。

![访客侧 SSE 实时人工回复](../_images/kefu-sse-realtime.png)

![访客转人工](../_images/kefu-visitor-takeover.png)

**管理端控制台（已交付 · SSE 实时）**：设置中心 → 智能客服（`/admin/kefu`） → 选实例 → 「人工接管」tab。左右主从：
- **左·会话队列**：过滤「全部 / 待人工 / 接管中」；每项显状态徽标（待人工=橙 / 接管中=蓝 / AI=灰）、访客 id、最后消息预览、接管人、时间；**SSE 实时刷新**（订阅事件总线的实例频道 `inst:<iid>`；短时一次性 ticket 鉴权）——访客转人工/发消息、任一管理员接管/回复/释放即时冒泡，无需轮询；仍可「🔄」手动刷新。
- **右·对话 + 操作**：点队列某会话看完整对话（访客右 / 客服左；人工消息带「人工」徽标）；未接管时点「接管」→ 变「释放」且回复框启用；输入人工回复发送即写入该会话（访客侧 SSE 即时收到）；点「释放」回到 AI。回复框**仅接管中可用**。打开的会话有新消息（访客发言/他人回复）时**实时**刷新。

> 至此人工接管 Phase 2 前端全链路（访客侧 + 管理端控制台）**已全 SSE 实时**：访客与管理端共享同一进程内事件总线，任一侧动作即时反映到另一侧，无轮询。

![管理端接管控制台](../_images/kefu-takeover-console.png)

![控制台实时（访客转人工即时冒泡）](../_images/kefu-console-realtime.png)
...
- **Phase 3 — 前端增强**：可嵌入 `kefu-widget.js`（独立构建 entry + iframe，不依赖宿主路由）、文件上传入访客对话。（访客全页 `/kefu/:slug` 已在 Stage ② 交付；`KefuManager.vue` 管理页已在设置中心「智能客服」（`/admin/kefu`）交付。）
