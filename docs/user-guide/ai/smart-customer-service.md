...
- **左栏 — AI 对话区**：顶部标题栏显示实例名称；欢迎语（以客服气泡形式、带头像展示）下方展示来自 `guided_questions` 的可点击提示气泡；历史消息流（见下「消息气泡外观」）；底部输入框 + 发送按钮。
...
![访客页消息气泡](../_images/kefu-message-bubbles.png)

对话区消息采用**双向气泡**布局，区分访客与客服：

- **访客消息**：右对齐，主色气泡（白色文字），无头像；附件以 `📎 文件名` 芯片形式显示在气泡内文字下方。
- **客服消息**：左对齐，白色卡片气泡（带浅色描边，在浅灰页面背景上清晰区隔），左侧圆形头像（优先使用 `branding.logo` 图片，否则取客服名称首字生成的彩色圆标）；内容 Markdown 渲染。
- **时间戳**：每条已入库消息在气泡下方显示 `HH:mm` 发送时间；本地乐观消息（尚未落库）不显示时间。
- **正在输入**：客服回复期间，对话区显示左侧三点动画气泡。

> 气泡配色全部基于 Element Plus 主题变量，随深浅色主题自动适配。
