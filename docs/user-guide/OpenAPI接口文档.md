...
> **用原 ID 重建记录可恢复"悬空引用"**：引用选择（quoteSelect）字段存的是目标记录的内部 ID。若目标记录被删除、源记录里的引用变成悬空，**用相同的 `id` 重新创建该目标记录**，悬空引用会立即恢复（源记录存的那个 ID 又有效了）。注意：用全新 ID 重建则无法恢复——源记录手里的旧 ID 既非主键值也非显示值，匹配不回去。
