...
在记录的 **查看** 弹窗里，**文件字段**的每个文件后面都有一个**下载图标**（任意格式都能下载）；**支持渲染的格式**点击文件名还会**在线预览**（预览弹窗内也带下载按钮）。预览与下载互不冲突：

| 类型 | 点击文件名 | 下载 |
|------|-----------|------|
| Word（.docx）、Excel（.xlsx/.xls）、PPT（.pptx）、PDF | 在线渲染预览 | ✅ |
| Markdown（.md）、文本（.txt/.json/.csv/.log/.xml/.yaml…） | 在线渲染预览 | ✅ |
| 图片（.png/.jpg/.svg…） | 在线显示 | ✅ |
| 其它格式（不支持渲染） | 直接下载 | ✅ |

> 预览组件按需懒加载，仅在首次打开预览时才加载渲染库，不影响首屏速度。

每个文件名前还会按**文件类型**显示一个**彩色小图标**，便于一眼区分：Excel（绿色表格）、Word（蓝色文档）、PPT（橙红）、PDF（红色）、图片（紫色）、Markdown/文本（蓝灰）、压缩包（橙色文件夹）、视频（蓝色摄像机）、音频（青色耳机），其余类型显示通用文档图标。
