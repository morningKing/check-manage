...
> **数据页增/改/删完全由「数据页 CRUD」权限决定，与角色名无关**：给**访客**（或任意自定义角色）勾选某数据页的增/改/删，该角色即可在该页执行对应操作——新增/编辑/删除按钮会按权限显示，后端也按页放行。
>
> **文件/图片字段上传**遵循同一套权限：只要角色对该数据页有**增或改**权限即可上传附件，无需额外的全局写权限。
