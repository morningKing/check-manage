...
在 **设置中心 → 系统设置**（`/admin/system-settings`）的「基本设置」Tab 中可定制系统的品牌信息与登录页文案，需要 `admin.system_config` 权限。修改保存后即时生效（登录页在下次打开时读取，无需重新部署）。
...
1. 进入 **设置中心 → 系统设置**（`/admin/system-settings`）。
...
在 **设置中心 → 系统设置**（`/admin/system-settings`）的「首页配置」Tab 中点击「**新增区块**」可添加以下区块（按角色可见、启用/停用）：
...
「首页配置」左侧是一个常驻的区块面板，展示所有可新增的区块类型：

- **拖拽**卡片到右侧网格的任意位置，区块会直接创建在松手的位置（默认半行宽，方便旁边再放一个）。
- **点击**卡片则按老规矩追加到网格最底部，不想精确定位时更快捷。

两种方式创建后都可以用已有的拖拽/缩放功能继续调整。

![首页区块面板拖拽新增](../_images/home-block-palette.png)

### 自由布局（拖拽 / 缩放）

「首页配置」Tab 用一个 12 列网格编辑器展示所有区块：

- **拖拽**卡片的空白区域可移动位置，让多个区块并排显示；**拖拽卡片右下角**可调整宽高。
- 调整后点击顶部的「**保存布局**」按钮才会生效（未保存时切换到「基本设置」Tab 会弹出确认提示）。
- 布局是**全局**的：管理员保存后，所有用户打开首页看到的都是同一份排布（仍受每个区块的「可见角色」/启用开关控制是否显示）。
- **移动端**（窄屏浏览器）不支持自由布局，会自动按阅读顺序（从左到右、从上到下）回退为单列纵向堆叠，无需额外配置。

![首页网格布局编辑器](../_images/home-layout-editor.png)

## 首页配置 · 快速录入表单

在 **设置中心 → 系统设置**（`/admin/system-settings`）的「首页配置」Tab 中可添加首页区块。其中「**快速录入**」区块在首页放一个入口，点击后弹出某个数据页的录入表单，提交即新增一条记录；区块内还会**展示该数据页最近 5 条记录**（提交后自动刷新）。
