# AI Chat 功能说明与方案设计（已归档）

> 📦 **本文档已归档。** 当前权威设计见 **[`../09-AI智能助手.md`](../09-AI智能助手.md)**（含后续补齐的
> 多智能体 @mention、批任务、定时 AI 流水线 ai_scan）。**平台 AI 权威设计以 [`../09-AI智能助手.md`](../09-AI智能助手.md) 为准。**本文保留早期单会话方案的设计背景与验证记录备查。
...
- **中间消息区**(居中文档列,约 780px):
  ...
  - **运行结果块**:点「运行」后,在对话流内插入一个可折叠的「运行结果」块,显示成功/出错/超时状态、stdout/stderr、以及产出文件的下载链接,让"执行→结果"在对话里连贯呈现。运行结果作为 `run_result` 类型的消息**持久化到 `ai_chat_messages`,刷新后仍在历史中**。
