...
> 取代旧版 `archive/concurrency-control-design.md`（仅含早期的单记录乐观锁）。
...
## 7. 场景五：工作流推进 · 实例并发与一致性

工作流引擎（`utils/workflow_engine.py`）挂在 `update_item` 状态转换之后，与状态字段写入**同一事务**：

- **下游记录生成** `spawn_record` 复用 `allocate_sequence` + `acquire_pk_lock`，与 `create_item` 同等序号/主键一致性。
- **实例并发** `on_transition` 对实例行 `SELECT … FOR UPDATE`：同一实例的并发推进/回退被串行化，后到者读到已变更的活动分支、转换不再匹配 → **不重复 spawn 下游**。
- **条件 DAG + 并行多分支（v2）**：定义为显式 `edges`（`{source,target,kind,condition?}`）。推进按出边路由——命中条件的边**全取（并行扇出）**，否则走默认（无条件）边；无 edges 时按 stages 顺序回退线性链（旧定义不变）。实例用 `active_stages` 追踪**并发活动分支**（一条记录可扇出多个下游、各自独立推进），`FOR UPDATE` 串行化对同一实例的并发改动；**所有活动分支结束**后实例才 `completed`。`active_stages` 为空的旧实例按 `current_stage_id` 兼容定位。
- **分支隔离**：`on_transition` 接收源记录的 `branch_id`，下游 spawn 与回退重置都落在**同一分支**，不再硬编码 `main`（避免分支内的流程把下游记录泄漏到主干）。
- **回退后重推进不重复 spawn**：回退不截断 chain；重新推进时若 chain 中已存在下一阶段项则**复用并重置其状态**，而非再 spawn 一条——杜绝重复下游记录与 chain 无限增长。
- **推进失败即回滚**：`on_transition` 内任何异常（角色不符抛 `WorkflowError`，或 spawn 出错）都使 `update_item` **回滚整笔事务**并返回 403/500——状态字段也不落库，杜绝「状态已改、实例未推进」的分裂态。
- **实例唯一**：`start_instance` 启动前校验同一记录无运行中实例（`find_running_instance_by_record`），否则 409，避免产生永远推进不了的孤儿实例。
- **定义删除保护**：存在运行中实例的工作流定义不可删除（409），避免在途流程从收件箱消失。
- **最终一致取舍**：下游 spawn 走直接 `INSERT`（同事务原子），**不**触发下游集合的校验脚本 / 触发器 / Webhook——这是有意的 MVP 取舍（避免递归触发风暴、保持 spawn 在上游事务内）。若下游需链式触发，应在 skill 侧经 MCP 处理。
...
```
详见 `docs/user-guide/integration/data-migration.md`。
